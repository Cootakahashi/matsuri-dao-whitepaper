...
**Matsuri App**을 열고, 성지 지도에서 신사나 사찰을 찾아 현장에 가서 체크인하세요. 활동은 **기여 점수**로 기록됩니다.
...
sequenceDiagram
    ...
    U->>GPS: 신사에 도착, "체크인" 탭
    GPS->>API: 좌표 + 증명 해시 전송
    API->>API: 위치 검증 + 기여 점수 기록
    API-->>U: 결과 표시: "⛩️ 체크인 완료!" + 오미쿠지 뽑기
    U->>API: 오미쿠지 뽑기
    API-->>U: "🏆 大吉! 보너스 점수!"
    API->>SC: Solana에 제출 (비동기, 8월 이후)
```

**핵심 원칙 — 방문자가 적은 곳일수록 더 많이 벌 수 있다:**

| 사이트 유형 | 예시 | 점수 |
| :--- | :--- | :---: |
| 🏙️ **주요** | 센소지, 기요미즈데라, 후시미이나리 | 표준 |
| 🌆 **지역 거점** | 각 현의 이치노미야, 지방 대사 | 더 높음 |
| 🏞️ **시골** | 역사 깊은 지방 신사 | 훨씬 높음 |
| ⛰️ **프론티어** | 산악 사찰, 외딴 섬 성지 | 최고 |

**추가 점수 요소:**
- **방문 빈도** — 정기적인 방문자는 시간이 지남에 따라 더 많이 획득
- **Omikuji** — 무작위 운세 뽑기로 보너스 점수 추가 (大吉이 최고!)
- **스폰서 사이트** — 지자체가 특정 사이트를 부스트 가능

:::info 기여 점수 → MTC
활동은 **기여 점수**로 축적됩니다. 각 반감기 에포크(2027년 6월 시작)마다 점수는 5.5억 마이닝 풀에서 MTC로 전환됩니다. 커뮤니티에 대한 기여도가 높을수록 더 많은 MTC를 받게 됩니다. 정확한 부스트 계수는 단계적으로 확정되어 스마트 컨트랙트에 구현됩니다 — 실제 풀 규모에 맞춘 공정한 배분을 보장합니다.
:::
...
기여 점수는 두 가지 요소를 기반으로 계산됩니다:
- **네트워크 도달**（30%）— 얼마나 많은 사람을 데려왔는가
- **경제 활동**（70%）— 추천 네트워크의 실제 구매

> **핵심 인사이트:** 점수의 대부분은 네트워크 내의 **실제 경제 활동**에서 산출되며, 단순한 가입 수가 아닙니다. 한 번도 소비하지 않는 1,000명을 초대하는 것보다 활발히 사용하는 10명을 초대하는 것이 더 많이 벌 수 있습니다.

점수는 시간이 지남에 따라 축적되며 각 반감기 에포크에서 MTC로 전환됩니다. 부스트 배율(예: MTC 스테이킹, 시즌 랭킹)은 스마트 컨트랙트를 통해 단계적으로 도입됩니다.
...
모든 어드벤처 마이닝 체크인에는 무료 Omikuji (오미쿠지) 뽑기가 포함됩니다 — 일반 점수에 더해지는 보너스입니다.

| 운세 | 희귀도 | 보너스 |
| :--- | :---: | :--- |
| 🏆 **大吉** (Great Blessing) | 레어 | 최고 보너스 점수 + NFT |
| ✨ **吉** (Blessing) | 언커먼 | 높은 보너스 점수 |
| 🌸 **小吉** (Small Blessing) | 커먼 | 소액 보너스 |
| 🍃 **末吉** (Future Blessing) | 커먼 | 참여 기록 |
| 💀 **凶** (Misfortune) | 언커먼 | 참여 기록 |

결과는 Solana의 **위변조 방지 커밋-리빌 프로토콜**에 의해 결정됩니다. 커밋 단계 이후에는 서버조차 결과를 변경할 수 없습니다. 정확한 확률과 보너스 금액은 스마트 컨트랙트 구현 시 확정됩니다.
...
| **📈 Toku 스테이킹** | MTC를 잠금하여 기여 점수 부스트 (최대 약 50% 부스트) | 🔜 2026년 8월 |
