...
:::info 状态: App Store 已发布 (v3.0)
面向普通用户的主力应用。活动预订、支付、Web3 钱包、AR 挖矿——一切在一个应用中完成。**现已在 App Store 上线。**
:::

<div style={{display: 'flex', justifyContent: 'center', margin: '2rem 0'}}>
  <a href="https://apps.apple.com/app/matsuri-%E7%A5%AD/id6761138366" target="_blank" rel="noopener noreferrer" aria-label="在 App Store 查看 Matsuri" className="app-store-badge">
...
> ⚠️ **状态说明。** 下方所示的三款程序是协议的**第 2 阶段设计
> 参考**,**尚未部署到主网**。链上安全工作以**分阶段审计**(Hashlock)
> 推进:第 1 阶段审计 `matsuri-buyback`(2026 年 8 月);第 1.5 阶段审计
> `matsuri-vesting` ＋ `matsuri-distribution`(2027 年 Q1)。该图描述的是目标
> 架构,而非当前部署状态。

Matsuri 的设计由 Solana 上的**三款 Anchor(Rust) 程序**构成,分别承担生态的各个支柱(第 2 阶段设计参考——尚未部署)。
