...
MTCは投機的なトークンではありません。すべてのアクションが価値を生み出し、価値を獲得するリアルエコノミーを循環しています。Webアプリケーションと管理ダッシュボードは**すでに稼働中**です。現在はオフチェーン（Django）で貢献スコアを記録しており、**監査ゲート付きの段階**でオンチェーンへ移行します——各コントラクトはセキュリティ監査に合格した後にのみメインネットへデプロイされます（下記の[オンチェーン移行ロードマップ](#on-chain-migration-roadmap)参照）。
...
4. 貢献スコアとして残高に反映
5. 後の監査済みフェーズにて：検証済みスコアをオラクル経由でオンチェーンに記録し、ブロックチェーン上で確認可能に

---

### 2. ⛩️ アドベンチャーマイニング（歩いて稼ぐ）

**プロジェクト「巡礼」 ── スマートコントラクト設計完成。メインネットデプロイは後の監査済みフェーズで（フェーズ1のバイバックコントラクトの後）**

![夜の聖地 ── 静寂の中で、おみくじが光になる](/brand/07_A_night_shrine.webp)

GPSとトークンインセンティブを活用し、物理的な「人の流れ」を制御する次世代機能です。聖地マップはMatsuri Webアプリで**すでに稼働中**です。現在はオフチェーンで貢献スコアを記録し、後のフェーズでこのコントラクトの監査済みメインネットデプロイ後にオンチェーン報酬配布が開始されます。
...
sequenceDiagram
    ...
    participant SC as Solana（後の監査済みフェーズ）
...
:::warning 現在はオフチェーン → 後の監査済みフェーズにオンチェーンへ移行
紹介コミッションは現在Django（PostgreSQL）で追跡され、銀行振込または暗号通貨で支払われています。後のフェーズで——独自のセキュリティ監査に合格した後——Solana上の**Matsuri Referralスマートコントラクト**に移行し、オンチェーンで監査可能な支払いが実現します。（最初に監査されオンチェーン化されるコントラクトは `matsuri-buyback` です。下記の[移行ロードマップ](#on-chain-migration-roadmap)参照。）
:::
...
| **📈 Toku ステーキング** | MTCをロックして貢献スコアをブースト（最大約50%ブースト） | 🔜 後のフェーズ |
...
:::info 監査段階のロールアウト ── 最初に何が出るか
この移行は**段階的セキュリティ監査（Hashlock）**によってゲートされており、その順序は固定のカレンダーではなく監査によって決まります：

- **`matsuri-buyback`**（収益 → MTCの自動買い戻し）が**最初に**オンチェーン化されるコントラクトです——2026年に監査（フェーズ1）、Raydium上場後にデプロイされます。
- **`matsuri-vesting`**（550Mの半減期放出プール）は**2027-06-01のグランドアンロック**前後（フェーズ1.5）に `matsuri-distribution` とともに続きます。
- 以下の**マイニングコントラクト**（referral / adventure / omikuji）は**後のフェーズ**で、それぞれ独自の監査に合格した後にのみデプロイされます。

下記の表のプロダクトフェーズ番号はロールアウトのビジョンを示すものであり、その日付はこの監査スケジュールに従って並べられています。
:::
...
graph LR
    ...
    subgraph "最初のオンチェーン（2026年、監査後）"
        H0["💱 収益バイバック → オンチェーン\n(matsuri-buyback contract)"]
    end
    subgraph "後の監査済みフェーズ（ハイブリッド）"
...
| **フェーズ2（2026年、監査ゲート付き）** | Hashlock監査後、最初のスマートコントラクトをメインネットへ | **`matsuri-buyback`** — 事業収益 → MTCの自動買い戻し |
| **フェーズ3（グランドアンロック、2027-06-01）** | 550Mプール起動 | **`matsuri-vesting`** — 550Mマイニングプールの半減期放出（`matsuri-distribution` とともに）；DAOガバナンス開始 |
| **フェーズ4（後ほど、それぞれ監査ゲート付き）** | マイニングコントラクトをメインネットへ | 紹介コミッション、アドベンチャーマイニング報酬、Omikuji抽選、オラクル経由メディアマイニング |
| **フェーズ5（2027年〜）** | 共創エコノミー | オンチェーンマーケットプレイス（地方特産品ショップ + GCFストア）、NFT権利付きクラウドファンディング、クリエイター + コミュニティ + バイバックへの自動収益分配 |
...
- **段階的監査（Hashlock）：** フェーズ1で `matsuri-buyback` を監査（2026年）；フェーズ1.5で `matsuri-vesting` ＋ `matsuri-distribution` を監査（2027-06-01のアンロック前後）。コントラクトは監査に合格した後にのみ、一つずつメインネットにデプロイされます
