...
:::info 执行这一过程的合约
在 2027-06-01 解禁时,550M 池将从第三方 **Streamflow** 锁仓流入 **`matsuri-vesting`** 智能合约,由其在链上执行减半释放。`matsuri-vesting` 与 `matsuri-distribution` 一同属于分阶段 **Hashlock** 审计(第 1.5 阶段)。首个上线的合约是 `matsuri-buyback`(第 1 阶段,2026 年)。
:::
