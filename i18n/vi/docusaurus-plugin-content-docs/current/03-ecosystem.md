...
![Một hệ sinh thái chơi bằng ba giọng](/brand/05b_B_triptych.webp)

MTC không chỉ là một token. Ba sản phẩm và một cộng đồng quốc tế cùng nhau xây dựng một nền kinh tế bảo vệ văn hóa.

```mermaid
graph TD
    A["🤝 GCF — cộng đồng quốc tế"] -->|Lên kế hoạch sự kiện| B["⛩️ Matsuri — ứng dụng trải nghiệm văn hóa"]
    A -->|Đăng nội dung| C["📰 J-Times — truyền thông văn hóa Nhật Bản"]
    B --> D["🪙 Kiếm và tiêu MTC"]
    C --> D
    D -->|Lưu thông qua nền kinh tế| A
```
