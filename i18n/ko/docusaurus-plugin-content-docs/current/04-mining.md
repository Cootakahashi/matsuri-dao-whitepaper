...
MTC는 투기적인 토큰이 아닙니다. 모든 액션이 가치를 만들어 내고, 가치를 획득하는 리얼 이코노미를 순환하고 있습니다. 웹 애플리케이션과 관리 대시보드는 **이미 가동 중**입니다. 현재는 오프체인(Django)에서 기여 스코어를 기록하고 있으며, **감사 게이트 단계**로 온체인으로 이행합니다——각 컨트랙트는 보안 감사에 합격한 후에만 메인넷에 배포됩니다(아래의 [온체인 이행 로드맵](#on-chain-migration-roadmap) 참조).
...
4. 기여 스코어로 잔액에 반영
5. 이후의 감사된 단계에서: 검증된 스코어를 오라클 경유로 온체인에 기록하여, 블록체인 위에서 확인 가능하게

---

### 2. ⛩️ 어드벤처 마이닝(걸으며 번다)

**프로젝트 "순례" ── 스마트 컨트랙트 설계 완성. 메인넷 배포는 이후의 감사된 단계에서(페이즈 1 바이백 컨트랙트 이후)**

![밤의 성지 ── 고요 속에서, 오미쿠지가 빛이 된다](/brand/07_A_night_shrine.webp)

GPS와 토큰 인센티브를 활용하여, 물리적 "사람의 흐름"을 제어하는 차세대 기능입니다. 성지 맵은 Matsuri 웹앱에서 **이미 가동 중**입니다. 현재는 오프체인에서 기여 스코어를 기록하고, 이후 단계에서 이 컨트랙트의 감사된 메인넷 배포 이후 온체인 보상 배포가 시작됩니다.
...
sequenceDiagram
    ...
    participant SC as Solana(이후의 감사된 단계)
...
:::warning 현재는 오프체인 → 이후의 감사된 단계에 온체인으로 이행
추천 커미션은 현재 Django(PostgreSQL)에서 추적되고, 은행 이체 또는 암호화폐로 지급되고 있습니다. 이후 단계에서——자체 보안 감사에 합격한 후——Solana 위의 **Matsuri Referral 스마트 컨트랙트**로 이행하여, 온체인에서 감사 가능한 지급이 실현됩니다.（최초로 감사되어 온체인화되는 컨트랙트는 `matsuri-buyback` 입니다. 아래의 [이행 로드맵](#on-chain-migration-roadmap) 참조.）
:::
...
| **📈 Toku 스테이킹** | MTC를 락해서 기여 스코어를 부스트(최대 약 50% 부스트) | 🔜 이후 단계 |
...
:::info 감사 단계별 롤아웃 ── 무엇이 먼저 나오는가
이 이행은 **단계적 보안 감사(Hashlock)** 로 게이트되어 있으며, 그 순서는 고정된 달력이 아니라 감사에 의해 결정됩니다:

- **`matsuri-buyback`**(수익 → MTC 자동 매수)가 **최초로** 온체인화되는 컨트랙트입니다——2026년에 감사(페이즈 1), Raydium 상장 이후 배포됩니다.
- **`matsuri-vesting`**(550M 반감기 방출 풀)은 **2027-06-01 그랜드 언락** 전후(페이즈 1.5)에 `matsuri-distribution` 과 함께 이어집니다.
- 아래의 **마이닝 컨트랙트**(referral / adventure / omikuji)는 **이후 단계**로, 각각 자체 감사에 합격한 후에만 배포됩니다.

아래 표의 프로덕트 페이즈 번호는 롤아웃 비전을 나타내며, 그 날짜는 이 감사 스케줄에 따라 배열되어 있습니다.
:::
...
graph LR
    ...
    subgraph "최초의 온체인(2026년, 감사 후)"
        H0["💱 수익 바이백 → 온체인\n(matsuri-buyback contract)"]
    end
    subgraph "이후의 감사된 단계(하이브리드)"
...
| **페이즈 2(2026년, 감사 게이트)** | Hashlock 감사 후 최초의 스마트 컨트랙트를 메인넷에 | **`matsuri-buyback`** — 사업 수익 → MTC 자동 매수 |
| **페이즈 3(그랜드 언락, 2027-06-01)** | 550M 풀 가동 | **`matsuri-vesting`** — 550M 마이닝 풀의 반감기 방출(`matsuri-distribution` 과 함께); DAO 거버넌스 개시 |
| **페이즈 4(이후, 각각 감사 게이트)** | 마이닝 컨트랙트를 메인넷에 | 추천 커미션, 어드벤처 마이닝 보상, Omikuji 추첨, 오라클 경유 미디어 마이닝 |
| **페이즈 5(2027년〜)** | 공창 이코노미 | 온체인 마켓플레이스(지방 특산품 숍 + GCF 스토어), NFT 권리 부여 크라우드펀딩, 크리에이터 + 커뮤니티 + 바이백으로의 자동 수익 분배 |
...
- **단계적 감사(Hashlock):** 페이즈 1에서 `matsuri-buyback` 을 감사(2026년); 페이즈 1.5에서 `matsuri-vesting` ＋ `matsuri-distribution` 을 감사(2027-06-01 언락 전후). 컨트랙트는 감사에 합격한 후에만, 하나씩 메인넷에 배포됩니다
