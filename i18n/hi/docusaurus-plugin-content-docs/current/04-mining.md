...
MTC सट्टेबाज़ी का टोकन नहीं है। यह एक असली अर्थव्यवस्था में बहता है जहाँ हर क्रिया मूल्य पैदा करती है और उसे पकड़ लेती है। web application और admin dashboard **पहले से live हैं**। अभी contribution scores off-chain (Django में) दर्ज होते हैं और **audit-गेटेड चरणों** में on-chain आते जाएँगे — हर contract अपना security audit पास करने के बाद ही mainnet पर जाता है (नीचे [on-chain migration roadmap](#on-chain-migration-roadmap) देखें)।
...
5. एक बाद के audited चरण में : सत्यापित scores oracle के ज़रिए on-chain दर्ज होंगे और blockchain पर सत्यापन-योग्य बनेंगे

---

### 2. ⛩️ Adventure माइनिंग (चलिए और कमाइए)

**प्रोजेक्ट "Junrei" — smart-contract डिज़ाइन पूर्ण; mainnet deployment एक बाद के audited चरण में (Phase 1 buyback contract के बाद)**

![पवित्र स्थल पर रात — सन्नाटे में omikuji ही रोशनी बन जाता है](/brand/07_A_night_shrine.webp)

एक अगली पीढ़ी की सुविधा जो GPS और टोकन प्रोत्साहनों से लोगों की भौतिक "गति-धारा" को गढ़ती है। सैकरेड-साइट मैप Matsuri web ऐप में **पहले से live है**। फ़िलहाल contribution scores off-chain दर्ज हैं; इस contract की audited mainnet deployment के बाद एक बाद के चरण में on-chain पुरस्कार वितरण शुरू होगा।
...
sequenceDiagram
    ...
    participant SC as Solana (बाद का audited चरण)
    ...
    API->>SC: Solana पर push (async, बाद का चरण)
```
...
:::warning अभी off-chain → एक बाद के audited चरण में on-chain में स्थानांतरित
Referral कमीशन फ़िलहाल Django (PostgreSQL) में ट्रैक होते हैं और bank transfer या crypto द्वारा भुगतान किए जाते हैं। एक बाद के चरण में — अपना security audit पास करने के बाद — ये Solana पर **Matsuri Referral smart contract** पर आ जाएँगे, जिससे on-chain, auditable payouts मिलेंगे। (पहला audited on-chain contract `matsuri-buyback` है; [migration roadmap](#on-chain-migration-roadmap) देखें।)
:::
...
| **📈 Toku staking** | MTC lock करके contribution score boost करें (लगभग 50% तक) | 🔜 बाद का चरण |
...
:::info Audit-चरणबद्ध rollout — पहले क्या आता है
यह स्थानांतरण एक **चरणबद्ध security audit (Hashlock)** से नियंत्रित है, इसलिए क्रम किसी तय कैलेंडर से नहीं, audits से तय होता है :

- **`matsuri-buyback`** (राजस्व → स्वचालित MTC buy-back) on-chain आने वाला **पहला** contract है — 2026 में audited (Phase 1), Raydium listing के बाद deployed।
- **`matsuri-vesting`** (550M halving-release pool) लगभग **2027-06-01 Grand Unlock** (Phase 1.5) पर, `matsuri-distribution` के साथ आता है।
- नीचे दिए **mining contracts** (referral / adventure / omikuji) **बाद के चरण** हैं, हर एक अपना audit पास करने के बाद ही deploy होता है।

नीचे की तालिका में product-phase संख्याएँ rollout के विज़न को दर्शाती हैं; उनकी तारीख़ें इसी audit अनुसूची के पीछे क्रमित हैं।
:::
...
graph LR
    ...
    subgraph "पहला on-chain (2026, post-audit)"
        H0["💱 Revenue buyback → on-chain\n(matsuri-buyback contract)"]
    end
    subgraph "बाद के audited चरण (hybrid)"
...
| **Phase 2 (2026, audit-gated)** | अपने Hashlock audit के बाद mainnet पर पहला smart contract | **`matsuri-buyback`** — व्यावसायिक राजस्व → स्वचालित MTC buy-back |
| **Phase 3 (Grand Unlock, 2027-06-01)** | 550M pool सक्रिय होता है | **`matsuri-vesting`** — 550M माइनिंग pool की halving-release (`matsuri-distribution` के साथ); DAO governance शुरू |
| **Phase 4 (बाद में, हर एक audit-gated)** | mainnet पर mining contracts | Referral कमीशन, adventure-mining पुरस्कार, Omikuji draws, oracle-आधारित media mining |
| **Phase 5 (2027+)** | सह-सर्जन अर्थव्यवस्था | On-chain marketplace (क्षेत्रीय विशेषता-दुकानें + GCF store), NFT अधिकारों सहित crowdfunding, creator + community + buyback को स्वचालित राजस्व-वितरण |
...
- **चरणबद्ध audit (Hashlock):** Phase 1 `matsuri-buyback` को audit करता है (2026); Phase 1.5 `matsuri-vesting` + `matsuri-distribution` को audit करता है (लगभग 2027-06-01 unlock के समय)। Contracts अपना audit पास करने के बाद ही, एक-एक करके mainnet पर तैनात होते हैं
