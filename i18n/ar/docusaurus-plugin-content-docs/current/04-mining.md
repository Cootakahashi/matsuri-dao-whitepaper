...
MTC ليس رمزًا للمضاربة. كل فعل يولّد قيمة ويمنحها، في اقتصاد حقيقي متدفّق. تطبيق الويب ولوحة الإدارة **يعملان الآن**. يُحفظ حاليًا نقاط المساهمة off-chain (Django)، ويُنقل إلى on-chain على **مراحل محكومة بالتدقيق** — كل عقد ينتقل إلى الشبكة الرئيسية فقط بعد اجتياز تدقيقه الأمني (انظر [خارطة طريق الانتقال إلى on-chain](#on-chain-migration-roadmap) أدناه).
...
5. في مرحلة مدقَّقة لاحقة: تُسجَّل النقاط المُتحقق منها عبر أوراكل على البلوكتشين، قابلة للتحقق على السلسلة

---

### 2. ⛩️ تعدين المغامرة (اكسب بالمشي)

**مشروع «الحج» — تصميم العقد الذكي مكتمل؛ النشر على الشبكة الرئيسية في مرحلة مدقَّقة لاحقة (بعد عقد buyback في المرحلة 1)**

![الأرض المقدسة ليلًا — في السكون، تتحوّل الـOmikuji إلى نور](/brand/07_A_night_shrine.webp)

ميزة الجيل القادم التي توظّف GPS وحوافز التوكن لتوجيه «تدفّق الناس» فعليًا. خريطة الأماكن المقدسة **تعمل الآن** في Matsuri Web. نسجّل النقاط off-chain حاليًا، ويبدأ توزيع المكافآت on-chain بعد نشر هذا العقد المدقَّق على الشبكة الرئيسية في مرحلة لاحقة.
...
sequenceDiagram
    ...
    participant SC as Solana (مرحلة مدقَّقة لاحقة)
    ...
    API->>SC: إرسال إلى Solana (غير متزامن، مرحلة لاحقة)
```
...
<div style={{display: 'flex', gap: '1.5rem', justifyContent: 'center', alignItems: 'center', flexWrap: 'wrap', margin: '2rem 0'}}>
  ...
  <div style={{flex: '1 1 260px', maxWidth: '400px'}}>
    <p style={{margin: 0, fontSize: '0.95rem', lineHeight: 1.6}}><strong>تعدين الزيارة</strong> — سجّل الحضور قرب المعبد، استكشف الطاقة بكاميرا AR، اسحب أوميكوجي لمكافأة MTC. مضاعفات من 1.0× إلى 10.0×.</p>
  </div>
</div>
...
:::warning حاليًا off-chain → الانتقال إلى on-chain في مرحلة مدقَّقة لاحقة
تُتبع عمولات الإحالة حاليًا على Django (PostgreSQL) وتُدفع عبر التحويل البنكي أو العملات المشفرة. في مرحلة لاحقة — بعد اجتيازها تدقيقها الأمني الخاص — تنتقل إلى **عقد Matsuri Referral الذكي** على Solana، ما يحقق مدفوعات قابلة للتدقيق on-chain. (أول عقد on-chain مدقَّق هو `matsuri-buyback`؛ انظر [خارطة طريق الانتقال](#on-chain-migration-roadmap).)
:::
...
| **📈 Toku Staking** | اقفل MTC لتعزيز نقاط المساهمة (حتى حوالي 50% تعزيز) | 🔜 مرحلة لاحقة |
...
:::info طرح متدرّج حسب التدقيق — ما الذي يُطرح أولًا
الانتقال محكوم بـ**تدقيق أمني متدرّج (Hashlock)**، فالترتيب تحدّده عمليات التدقيق لا تقويم ثابت:

- **`matsuri-buyback`** (الإيرادات → إعادة شراء MTC تلقائيًا) هو **أول** عقد ينتقل إلى on-chain — مدقَّق في 2026 (المرحلة 1)، يُنشَر بعد إدراج Raydium.
- **`matsuri-vesting`** (مجمع الإطلاق النصفي البالغ 550M) يتبعه حول **Grand Unlock في 2027-06-01** (المرحلة 1.5)، مع `matsuri-distribution`.
- **عقود التعدين** أدناه (referral / adventure / omikuji) هي **مراحل لاحقة**، يُنشَر كل منها فقط بعد اجتياز تدقيقه الخاص.

أرقام مراحل المنتج في الجدول أدناه تصف رؤية الطرح؛ وتواريخها مرتّبة خلف جدول التدقيق هذا.
:::
...
graph LR
    ...
    subgraph "أول عقد on-chain (2026، بعد التدقيق)"
        H0["💱 إعادة شراء الإيرادات → on-chain\n(matsuri-buyback contract)"]
    end
    subgraph "مراحل مدقَّقة لاحقة (Hybrid)"
...
| **المرحلة 2 (2026، محكومة بالتدقيق)** | أول عقد ذكي على الشبكة الرئيسية، بعد تدقيق Hashlock له | **`matsuri-buyback`** — إيرادات الأعمال → إعادة شراء MTC تلقائيًا |
| **المرحلة 3 (Grand Unlock، 2027-06-01)** | تفعيل مجمع 550M | **`matsuri-vesting`** — الإطلاق النصفي لمجمع التعدين البالغ 550M (مع `matsuri-distribution`)؛ تبدأ حوكمة DAO |
| **المرحلة 4 (لاحقًا، كل منها محكوم بالتدقيق)** | عقود التعدين على الشبكة الرئيسية | عمولات الإحالة، مكافآت تعدين المغامرة، سحب Omikuji، تعدين الإعلام عبر oracle |
| **المرحلة 5 (2027〜)** | اقتصاد إبداع مشترك | سوق on-chain (متاجر منتجات إقليمية + متجر GCF)، تمويل جماعي بحقوق NFT، توزيع تلقائي للإيرادات على المبدع + المجتمع + buy-back |
...
- **تدقيق متدرّج (Hashlock):** المرحلة 1 تدقّق `matsuri-buyback` (2026)؛ المرحلة 1.5 تدقّق `matsuri-vesting` + `matsuri-distribution` (حول فتح 2027-06-01). تُنشَر العقود على الشبكة الرئيسية واحدًا تلو الآخر، فقط بعد اجتياز تدقيقها
