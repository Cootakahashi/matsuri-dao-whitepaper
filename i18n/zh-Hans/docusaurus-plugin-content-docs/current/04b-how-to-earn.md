...
打开 **Matsuri App**，在圣地地图上找到神社或寺庙，前往现场签到。您的活动将被记录为**贡献分数**。
...
**额外分数因素：**
- **访问频率** — 定期访问者随时间推移赚取更多
- **Omikuji** — 随机御神签抽取可增加奖励分数（大吉最好！）
- **赞助圣地** — 地方政府可以提升特定圣地的分数

:::info 贡献分数 → MTC
您的活动以**贡献分数**形式累积。在每个减半纪元（2027年6月起），分数将从5.5亿MTC挖矿池中转换为MTC。您对社区的贡献越多，获得的MTC就越多。确切的加成系数将逐步确定并实施到智能合约中——确保与实际矿池规模相匹配的公平分配。
:::
...
> **关键洞察：** 分数的大部分来自网络中的**真实经济活动**，而非简单的注册量。邀请1,000个从不消费的人，收益远不如邀请10个活跃用户。

分数随时间累积，在每个减半纪元转换为MTC。加成倍率（如质押MTC、赛季排名）将通过智能合约逐步引入。
...
每次探险挖矿签到都包含一次免费的Omikuji（御神签）抽取——在常规分数之上的额外奖励。

| 运势 | 稀有度 | 奖励 |
| :--- | :---: | :--- |
| 🏆 **大吉**（Great Blessing） | 稀有 | 最高奖励分数 + NFT |
| ✨ **吉**（Blessing） | 非常见 | 高奖励分数 |
| 🌸 **小吉**（Small Blessing） | 常见 | 小额奖励 |
| 🍃 **末吉**（Future Blessing） | 常见 | 参与记录 |
| 💀 **凶**（Misfortune） | 非常见 | 参与记录 |

结果由 Solana 上的**防篡改承诺-揭示协议**决定。在承诺阶段之后，即使服务器也无法更改您的结果。确切的概率和奖励数额将在智能合约实施时确定。
...
| **📈 Toku 质押** | 锁定MTC以提升贡献分数（最高约50%加成） | 🔜 2026年8月 |
