...
| **Matsuri** | التطبيق الرئيسي للمستخدم | ✅ مُصدَر | 🇯🇵🇬🇧🇨🇳🇹🇭🇳🇴 |
...
:::info الحالة: مُصدَر على App Store (v3.0)
التطبيق الرئيسي للمستخدم العادي. يكتمل كل شيء في تطبيق واحد: حجز الفعاليات، الدفع، محفظة Web3، AR Mining. **متوفر الآن على App Store.**
:::
...
> ⚠️ **ملاحظة الحالة.** البرامج الثلاثة الموضّحة أدناه هي **مرجع تصميم المرحلة 2**
> للبروتوكول و**لم تُنشَر بعد على الشبكة الرئيسية**. يسير العمل الأمني on-chain كـ**تدقيق
> متدرّج** (Hashlock): المرحلة 1 تدقّق `matsuri-buyback` (المرحلة 1)؛ المرحلة 1.5 تدقّق
> `matsuri-vesting` + `matsuri-distribution` (المرحلة 1.5). يصف المخطط البنية المستهدفة،
> لا حالة النشر الحالية.

تصميم Matsuri يتألّف من **ثلاثة برامج Anchor (Rust)** على Solana، كل منها يحمل ركنًا من أركان النظام البيئي (مرجع تصميم المرحلة 2 — لم يُنشَر بعد).
