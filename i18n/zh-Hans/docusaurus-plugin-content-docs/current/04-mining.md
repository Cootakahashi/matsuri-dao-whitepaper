...
MTC 不是一枚投机代币。所有行为都会创造价值、获取价值,形成一个真实经济的循环。Web 应用与管理仪表盘**已在运行中**。当前贡献分数在链下(Django)记录,将以**审计门控的阶段**迁移到链上——每份合约只有在通过其安全审计后才会部署到主网(参见下方的[链上迁移路线图](#on-chain-migration-roadmap))。
...
3. 网络恢复后发送至服务器进行校验
4. 作为贡献分数计入余额
5. 在后续的审计阶段:通过预言机将已核验的分数上链,可在区块链上查验

---

### 2. ⛩️ 冒险挖矿(靠行走赚取)

**"巡礼"项目 ── 智能合约设计已完成;主网部署将在后续的审计阶段(在第 1 阶段回购合约之后)**

![夜之圣地 ── 寂静之中,御神签化为光](/brand/07_A_night_shrine.webp)

这是借助 GPS 与代币激励,对物理"人流"进行调控的新一代功能。圣地地图在 Matsuri Web 应用中**已上线**。当前贡献分数在链下记录,将在后续阶段、于本合约经审计的主网部署之后启动链上奖励发放。
...
sequenceDiagram
    ...
    participant SC as Solana(后续审计阶段)
...
<div style={{display: 'flex', gap: '1.5rem', justifyContent: 'center', alignItems: 'center', flexWrap: 'wrap', margin: '2rem 0'}}>
  <img src="/img/apps/matsuri-sacred-map-live.webp" alt="Matsuri 圣地地图 —— 周边神社及实时奖励倍数" style={{width: '100%', maxWidth: '260px', borderRadius: '20px', border: '1px solid rgba(167,139,250,0.2)', boxShadow: '0 8px 32px rgba(0,0,0,0.25)'}} />
  <div style={{flex: '1 1 260px', maxWidth: '400px'}}>
    <p style={{margin: 0, fontSize: '0.95rem', lineHeight: 1.6}}><strong>圣地地图</strong> —— 一眼掌握周边的神社及其实时倍数(例如明治神宫 ×2.5,边远圣地 ×8.0)。每天可签到三次,每 24 小时重置一次 —— 地图会引导你前往更稀有、奖励更高的地点。</p>
  </div>
</div>
...
:::warning 目前在链下 → 后续的审计阶段迁移至链上
推荐分成目前由 Django(PostgreSQL)追踪,通过银行转账或加密货币支付。在后续阶段——在其自身的安全审计通过之后——将迁移到 Solana 上的 **Matsuri Referral 智能合约**,实现可审计的链上支付。(首个经审计上链的合约是 `matsuri-buyback`;参见[迁移路线图](#on-chain-migration-roadmap)。)
:::
...
| **📈 Toku 质押** | 锁定 MTC 以加成贡献分数(最多约 50% 加成) | 🔜 后续阶段 |
...
:::info 审计分阶段上线 ── 谁先上线
本次迁移以**分阶段安全审计(Hashlock)**为门控,因此顺序由审计驱动,而非固定日历:

- **`matsuri-buyback`**(收入 → 自动回购 MTC)是**首个**上链的合约——于 2026 年审计(第 1 阶段),在 Raydium 上市后部署。
- **`matsuri-vesting`**(550M 减半释放池)将在 **2027-06-01 大解禁**前后(第 1.5 阶段)随 `matsuri-distribution` 一起跟进。
- 下方的**挖矿合约**(referral / adventure / omikuji)属于**后续阶段**,各自只有在通过自身审计后才会部署。

下表中的产品阶段编号描述的是上线愿景;其日期排在该审计时间表之后。
:::
...
| **阶段 2(2026 年,审计门控)** | 经 Hashlock 审计后,首个智能合约上主网 | **`matsuri-buyback`** — 业务收入 → 自动回购 MTC |
| **阶段 3(大解禁,2027-06-01)** | 550M 池启动 | **`matsuri-vesting`** — 550M 挖矿池的减半释放(随 `matsuri-distribution`);DAO 治理启动 |
| **阶段 4(后续,各自审计门控)** | 挖矿合约上主网 | 推荐分成、冒险挖矿奖励、御神签抽奖、通过预言机的媒体挖矿 |
| **阶段 5(2027 年起)** | 共创经济 | 链上市集(地方特产店 + GCF 商店)、带 NFT 权益的众筹、面向创作者 + 社区 + 回购的自动收益分配 |
...
- **分阶段审计(Hashlock):** 第 1 阶段审计 `matsuri-buyback`(2026 年);第 1.5 阶段审计 `matsuri-vesting` ＋ `matsuri-distribution`(在 2027-06-01 解禁前后)。合约只有在通过审计后才会逐一部署到主网
