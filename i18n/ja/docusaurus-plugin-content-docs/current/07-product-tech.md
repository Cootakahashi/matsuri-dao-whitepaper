...
<div style={{display: 'flex', justifyContent: 'center', margin: '2rem 0'}}>
  <a href="https://apps.apple.com/jp/app/gcf/id6761515025" target="_blank" rel="noopener noreferrer" aria-label="GCFをApp Storeで見る" className="app-store-badge">
...
:::info ステータス: App Store リリース済み (v3.0)
一般ユーザー向けのメインアプリ。イベント予約、決済、Web3ウォレット、ARマイニングまで、すべてを一つのアプリで完結。**App Store にて配信開始しました。**
:::

<div style={{display: 'flex', justifyContent: 'center', margin: '2rem 0'}}>
  <a href="https://apps.apple.com/app/matsuri-%E7%A5%AD/id6761138366" target="_blank" rel="noopener noreferrer" aria-label="MatsuriをApp Storeで見る" className="app-store-badge">
...
<div style={{display: 'flex', justifyContent: 'center', margin: '2rem 0'}}>
  <a href="https://apps.apple.com/jp/app/j-times/id6761513732" target="_blank" rel="noopener noreferrer" aria-label="J-TimesをApp Storeで見る" className="app-store-badge">
...
> ⚠️ **ステータス注記。** 以下に示す3つのプログラムはプロトコルの**フェーズ2の設計
> リファレンス**であり、**まだメインネットにデプロイされていません**。オンチェーンのセキュリティ作業は
> **段階的監査**（Hashlock）として進行します：フェーズ1で `matsuri-buyback` を監査（2026年8月）；フェーズ1.5で
> `matsuri-vesting` ＋ `matsuri-distribution` を監査（2027年Q1）。この図は目標とする
> アーキテクチャを示すものであり、現在のデプロイ状態ではありません。

Matsuri の設計は、エコシステムの各柱を担う Solana上の**3つのAnchor（Rust）プログラム**で構成されます（フェーズ2の設計リファレンス——まだデプロイされていません）。
